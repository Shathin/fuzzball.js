(function () {
  ...
  var processing = require("./lib/process.js")(
    clone_and_set_option_defaults,
    _isArray,
    QRatio,
    extract
  );
  ...
  function WRatio(str1, str2, options_p) {
    ...
    if (try_partial) {
      ...
      var ptsor =
        partial_token_sort_ratio(str1, str2, options) *
        unbase_scale *
        partial_scale;
      var ptser =
        partial_token_set_ratio(str1, str2, options) *
        unbase_scale *
        partial_scale;
  ...
  function extract(query, choices, options_p) {
    ...
    var tset = false;
    if (
      options.scorer.name === "token_sort_ratio" ||
      options.scorer.name === "partial_token_sort_ratio"
    ) {
      var proc_sorted_query = process_and_sort(query);
      tsort = true;
    } else if (
      options.scorer.name === "token_set_ratio" ||
      options.scorer.name === "partial_token_set_ratio"
    ) {
    ...
    _forEach(choices, function (value, key) {
      ...
      if (tsort) {
        ...
        if (value && value.proc_sorted) mychoice = value.proc_sorted;
          ...
          mychoice = process_and_sort(
            normalize ? mychoice.normalize() : mychoice
          );
        }
        ...
        if (value && value.tokens) {
          ...
          options.tokens = [
            query_tokens,
            tokenize(normalize ? mychoice.normalize() : mychoice, options),
          ];
        }
        ...
        if (typeof mychoice !== "string" || mychoice.length === 0)
          anyblank = true;
        if (normalize && typeof mychoice === "string")
          mychoice = mychoice.normalize();
        result = options.scorer(query, mychoice, options);
      }
      if (result > options.cutoff) {
        if (options.returnObjects)
          results.push({ choice: value, score: result, key: key });
  ...
  function _token_set(str1, str2, options) {
    ...
    if (options.sortBySimilarity) {
      var orderedTokenLists = _order_token_lists(
        str1,
        sorted_1to2List,
        str2,
        sorted_2to1List
